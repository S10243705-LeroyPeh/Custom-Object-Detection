...
def main(_argv):
    ...
    if not video_file_buffer:
        ...
        file_type = video_file_buffer.type.split('/')[0]
        if file_type == 'image':
            image = Image.open(tfflie.name)
    ...
    if vid:
        ...
        if FLAGS.crop:
            final_path = os.path.join(os.getcwd(), 'detections', 'crop', 'image')
            try:
                os.mkdir(final_path)
            except FileExistsError:
                pass
            crop_objects(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), pred_bbox, final_path, allowed_classes)

        if FLAGS.count:
            counted_classes = count_objects(pred_bbox, by_class=False, allowed_classes=allowed_classes)
            for key, value in counted_classes.items():
                print(f"Number of {key}s: {value}")
            image = utils.draw_bbox(image, pred_bbox, FLAGS.info, counted_classes, allowed_classes=allowed_classes, read_plate=FLAGS.plate)
        else:
            image = utils.draw_bbox(image, pred_bbox, FLAGS.info, allowed_classes=allowed_classes, read_plate=FLAGS.plate)
